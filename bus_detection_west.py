...
first_thres_west = 812
final_thres_west = 782
...
# Add this section to introduce the 10-minute delay
...
while True:
    ...
    for index,row in px.iterrows():
        ...
        if name == 'bus' and confidence_score >= 0.4:
            list.append([x1,y1,x2,y2])
            vehicle.append(name)
    bbox4_idx=tracker.update(list)
    count_vehicles(bbox4_idx,counter,first_thres_dn,final_thres_dn,check_thres_dn,first_thres_east,final_thres_east,first_thres_west,final_thres_west,vehicle,frame,'bus')
    ...
    bus=(len(counter))
    cvzone.putTextRect(frame,f'bus:-{bus}',(19,120),2,1)
