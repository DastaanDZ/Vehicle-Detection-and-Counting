        ...
        for i in vehicle:
            ...
            if id1 in vh_west:
                ...
                if cxm <  final_thres_west :
                    vh_west_conf.add(id1)

            # Checking vehicles in east direction
            if cxm > first_thres_east and cxm < final_thres_east and id1 not in vh_dn and id1 not in vh_west:
...
while True:
    ...
    for index,row in px.iterrows():
        ...
        if name == 'motorcycle' and confidence_score >= 0.1:
            list.append([x1,y1,x2,y2])
            vehicle.append(name)
    bbox4_idx=tracker.update(list)
    count_vehicles(bbox4_idx,counter,first_thres_dn,final_thres_dn,check_thres_dn,first_thres_east,final_thres_east,first_thres_west,final_thres_west,vehicle,frame,'motorcycle')
    ...
    cv2.line(frame, (first_thres_east, 254), (first_thres_east,497), (0, 255,0), 2)
    cv2.line(frame, (final_thres_east, 254), (final_thres_east,497), (0, 255,0), 2)
    # Threshold for covering vehicle in west direction
    cv2.line(frame, (first_thres_west ,283), (first_thres_west, 499), (255, 0,0), 2)
    cv2.line(frame, (final_thres_west,283), (final_thres_west, 499), (255, 0,0), 2)

    motorcycle=(len(counter))
    cvzone.putTextRect(frame,f'motorcycle:-{motorcycle}',(19,120),2,1)
